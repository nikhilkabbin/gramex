...
def vue(handler, path: str):
    ...
    vue_dir = join(cache_dir, f'vue-{cache_key}')

    # Recompile if output cache_file is missing, or path has been updated
    if not os.path.exists(vue_dir) or os.stat(path).st_mtime > os.stat(vue_dir).st_mtime:
        # Compile Vue file
        ...
        proc = yield gramex.service.threadpool.submit(subprocess.run, [
            ...
            vue_path, 'build', '--target', 'wc', filename, '--dest', vue_dir
    ...
    if 'map' in handler.args:
        ...
        return gramex.cache.open(os.path.join(vue_dir, target + '.map'), 'bin', mode='rb')
    else:
        # Serve compiled JS if browser requested just .vue
        handler.set_header('Content-Type', 'text/javascript')
        content = gramex.cache.open(os.path.join(vue_dir, target), 'bin', mode='rb')
